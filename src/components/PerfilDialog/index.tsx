...
export default function PerfilDialog({
  ...
  const preencherPerfil = (perfil: PerfilModel) => {
    ...
    setCategorias((c) => {
      ...
      return categorias.map((categoria) => {
        ...
        const permissoes =
          perfil.categoriasPermissao
            ?.find((pc) => pc.categoria == categoria.categoria)
            ?.permissoes?.map((p) => p.codigo) ?? [];
        novaCategoria.permissoes = novaCategoria.permissoes.map((p) => {
          return {
            ...p,
            selecionada: (p.selecionada = permissoes.includes(p.codigo)),
          };
        });
        novaCategoria.selecionada = novaCategoria.permissoes.every(
          (p) => p.selecionada
        );
        ...
        return fetchObtemPerfil(id)
          ...
          .catch((error) =>
            notificationApi.error({
              message:
                "Falha na busca pelo perfil. " + (error?.response?.data ?? ""),
            })
          );
      })
      .catch((error) =>
        notificationApi.error({
          message:
            "Falha na listagem de permissoes. " + (error?.response?.data ?? ""),
        })
      )
  ...
  const salvarPerfil = () => {
    ...
    const permissoes = categorias.flatMap((c) =>
      c.permissoes.filter((p) => p.selecionada).map((p) => p.codigo)
    );
    const perfil = {
      nome: nome.trim(),
      permissoes,
    ...
    if (!id) {
      fetchCadastroPerfil(perfil)
        ...
        .catch((error) => {
          notificationApi.error({
            message:
              "Falha no cadastro de perfil. " + (error?.response?.data ?? ""),
            duration: 30,
          });
        })
        .finally(() => setLoading(false));
    ...
    fetchAtualizaPerfil(id, perfil)
      ...
      .catch((error) =>
        notificationApi.error({
          message:
            "Falha na edição do perfil. " + (error?.response?.data ?? ""),
          duration: 30,
        })
      )
      .finally(() => setLoading(false));
  ...
  const selecionarTodasPermissoes = (index: number) => {
    ...
    nova[index].permissoes.forEach(
      (p) => (p.selecionada = nova[index].selecionada)
    );
    setCategorias(nova);
  };
  ...
  const selecionarPermissao = (categoria: number, permissao: number) => {
    ...
    novaCategoria.permissoes[permissao].selecionada =
      !novaCategoria.permissoes[permissao].selecionada;
    novaCategoria.selecionada = novaCategoria.permissoes.every(
      (p) => p.selecionada
    );
      ...
      <div className="d-flex justify-content-between">
        {id && (
          <h4 className="text-center mt-1">
            {readOnly ? "Detalhes do Perfil" : "Edição de Perfil"}
          </h4>
        )}
        ...
        >
          <section className="d-flex align-items-center w-100">
            <div className="input-label mt-2">
              <label className="mr-2">Perfil:</label>
            </div>
          ...
          <section className="mt-4">
            ...
            {categorias.map((p, indexCategoria) => (
              <Collapse
                ...
                readOnly={!canEdit}
                titulo={p.categoria}
                selected={p.selecionada}
                onSelect={() => selecionarTodasPermissoes(indexCategoria)}
              >
                {p.permissoes.map((permissao, indexPermissao) => (
                  <CollapseItem
                    key={`${p.categoria}-${permissao.codigo}`}
                    readOnly={!canEdit}
                    selected={permissao.selecionada}
                    onSelect={() =>
                      selecionarPermissao(indexCategoria, indexPermissao)
                    }
                  >
                    {permissao.descricao}
                  </CollapseItem>
